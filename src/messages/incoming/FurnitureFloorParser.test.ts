...
class TestReader {
  ...
  readLong(): number { return this.values[this.index++] as number; }
...
const variableParsed = new FurnitureFloorParser().parse(new TestReader([
  0, 2,
  FAKE_ITEM_A, 100, 10, 18, 0, "0.1", "0.1", 0,
  6, "0",
  "board", 0, "Users", "Score", 0, 0, 1,
  99, 1, "Ace",
  -1, 1, FAKE_OWNER,
  FAKE_ITEM_B, 101, 11, 19, 0, "0.1", "0.1", 0,
  0, "4",
...
const variableItem = variableParsed.items[0];
assert(variableItem.objectData.dataType === "highScore", "variable highscore should be detected");
assert(variableItem.objectData.dataType === "highScore" && variableItem.objectData.entries[0]?.score === 99, "variable highscore entries should stay aligned");
assert(variableParsed.items[1]?.id === FAKE_ITEM_B, "item after variable highscore should stay aligned");
